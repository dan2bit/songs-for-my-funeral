...
def fetch_channel_uploads(youtube, date_str=None):
    ...
    Strategy:
      1. Fetch all uploads (includes private, excludes still-processing).
      ...
      3. Fallback: if primary returns nothing, match by show date string in
         the video description (same date_variants logic as youtube_videos.tsv).
         This handles backfill where videos were uploaded weeks after the show.
...
def main():
    ...
    if args.new_show:
        ...
        if new_show_arg.startswith("since:"):
            ...
            # Collect all shows on or after since_date with no playlist URL, sorted by date
            queue = sorted(
                [
