...
//
// These are actual PNG images — not shapes — so PowerPoint's
// "Change Picture" (right-click menu) works immediately on first use.
// No delete-and-reinsert needed.
//
// Sized to exactly fill their slide areas at 96 dpi:
//   Landscape: 10" × 4.775"  →  960 × 458 px
//   Portrait:  6.65" × 5.625" →  638 × 540 px
// ─────────────────────────────────────────────────────
const PLACEHOLDER_LANDSCAPE = 'ppt/placeholder-landscape.png';
const PLACEHOLDER_PORTRAIT  = 'ppt/placeholder-portrait.png';
...
chapters.forEach(ch => {
  ...
  ch.tracks.forEach(tNum => {
    ...
    // ── PHOTO SLIDE TEMPLATE: LANDSCAPE ──
    {
      ...
      // Placeholder PNG — right-click → Change Picture in PowerPoint to replace
      slide.addImage({ path: PLACEHOLDER_LANDSCAPE, x: 0, y: 0, w: W, h: photoH,
    ...
    // ── PHOTO SLIDE TEMPLATE: PORTRAIT / SQUARE ──
    {
      ...
      // Placeholder PNG — right-click → Change Picture in PowerPoint to replace
      slide.addImage({ path: PLACEHOLDER_PORTRAIT, x: 0, y: 0, w: SIDEBAR_X - 0.15, h: H,
